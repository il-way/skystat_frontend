...
export default function Dashboard() {
  ...
  const { data: avg, isFetching: avgIsFetching, error: avgError, refetch: avgRefetch } = useQuery({
    ...
    queryFn: async () => MetarStatisticApi.fetchAverageSummary(basicQueryParams),
    enabled: false,
  ...
  const { data: tableRows, isFetching: tableIsFetching, error: tableError, refetch: tableRefetch } = useQuery({
    ...
    queryFn: async () => MetarStatisticApi.fetchDashboadTableSummary(basicQueryParams),
    enabled: false,
    placeholderData: keepPreviousData,
  });
    ...
    <div className="flex h-screen w-full bg-background text-foreground">
      ...
      <div className="flex-1 flex flex-col">
        ...
        <Topbar icao={icao} setIcao={setIcao}
                ...
                to={to} setTo={setTo}
                loading={loading} 
                isFetching={avgIsFetching} onFetch={handleFetch}
        />
        ...
        <main className="max-w-7xl mx-auto px-4 py-6 space-y-6">
          ...
          {/* Table */}
          <DashboardTable icao={icao} from={from} to={to} rows={tableRows || []} />
